...
float temp;
  ...
  *bp=*bp+258;
  ...
  for(k=-256;k<257;k++)
  {
    ...
    //*************************** 
    //printf("%f\n",-temp); 
    //temp=100.0*exp(-temp);//ORIG
    temp=100.0*taylor_exp(-temp);//TAYLOR_EXP_T1
    //***************************
    *(*bp+k)= (uchar)temp;
  }
}
...
/* }}} */
  ...
  for(i=-mask_size; i<=mask_size; i++)
    ...
    {
      //*******************************************************
      //x = (int) (100.0 * exp( ((float)((i*i)+(j*j))) / temp ));//ORIG
      x = (int) (100.0 * taylor_exp( ((float)((i*i)+(j*j))) / temp ));//TAYLOR_EXP_T2
      //*******************************************************
      *dpt++ = (unsigned char)x;
    }
