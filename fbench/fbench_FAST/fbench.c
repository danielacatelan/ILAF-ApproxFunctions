...
static inline float fast_tan (float x)
{
  static const float halfpi = 1.5707963267948966f;
  return 
      fast_sin_T1 (x) / fast_sin_T1 (x + halfpi);//ORIG
        
      //fast_sin (x) / fast_sin (FADDX(x, halfpi));//C1
} 
...
#define cot(x) (1.0 / fast_tan(x))//FAST_TAN_t9
...
static void transit_surface() {
        ...
        if (radius_of_curvature != 0.0) {//if3
              ...
              iang_sin = ray_height / radius_of_curvature;
               ...
               /*iang_sin = ((object_distance - radius_of_curvature) / 
                ...
                iang_sin = ((object_distance - radius_of_curvature) / radius_of_curvature) *
                    fast_sin(axis_slope_angle);//*///FAST_SIN_t3         
           }
           iang = asin(iang_sin);
           ...
           sagitta = fast_sin((old_axis_slope_angle + iang) / 2.0);//FAST_SIN_t4
           ...
           /*object_distance = ((radius_of_curvature * 
            ...
            object_distance = ((radius_of_curvature * 
                               fast_sin(old_axis_slope_angle + iang)) *
                               cot(axis_slope_angle)) + sagitta;//FAST - SENO_t5 e TAN_t9 */
           return;
        ...
        /*rang = -asin((from_index / to_index) *
          ...
          rang = -asin((from_index / to_index) *
                fast_sin(axis_slope_angle));//FAST_SIN_t6*/        
                
           
        /*object_distance = object_distance * ((to_index *
           cos(-rang)) / (from_index * cos(axis_slope_angle)));//ORIG*/
        /*object_distance = object_distance * ((to_index *
           fast_cos(-rang)) / (from_index *
           cos(axis_slope_angle)));//FAST_COS_t1 */ 
          object_distance = object_distance * ((to_index *
           fast_cos(-rang)) / (from_index *
           fast_cos(axis_slope_angle)));//FAST_COS_t2 */ 
...
{//main
        ...
        //for (itercount = 0; itercount < niter; itercount++) {//for1
           ...
           aberr_osc = 1.0 - (od_sa[1][0] * od_sa[1][1]) /
              (fast_sin(od_sa[0][1]) * od_sa[0][0]);//FAST_SIN_t7 */ 
           //*********************************************************  
           aberr_lchrom = od_fline - od_cline;
           //*********************************************************
           //max_lspher = sin(od_sa[0][1]);//ORIG
           max_lspher = fast_sin(od_sa[0][1]);//FAST_SIN_t8
