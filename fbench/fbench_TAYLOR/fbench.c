...
//*************************************
  //t = x + (xx / 3.0f);//-ORIG
  t = FADDX(x, (xx / 3.0f));//C1
//*************************************
  aux = (xx = xx * x * x); //x^5-ORIG 
  t = t + ((2.0f * aux) / 15.0f);//-ORIG
  
  aux = (xx = xx * x * x);//x^7-ORIG
  t = t + ((17.0f * aux) / 315.0f);//-ORIG

  aux = (xx = xx * x * x); //x^9//-ORIG
  t = t + ((62.0f * aux) / 2835.0f);//-ORIG

  aux = (xx = xx * x * x); //x^11//-ORIG
  t = t + ((1382.0f * aux) / 155925.0f);//*///-ORIG

  aux = (xx = xx * x * x); //x^13//-ORIG
  t = t + ((21844.0f * aux) / 6081075.0f);//*///-ORIG

  aux = (xx = xx * x * x); //x^15//-ORIG
  t = t + ((929569.0f * aux) / 638512875.0f);//*///-ORIG

  aux = (xx = xx * x * x); //x^17//-ORIG
  t = t + ((6404582 * aux) / 10854718875);//*///-ORIG

  aux = (xx = xx * x * x); //x^19//-ORIG 
  t = t + ((443861162.0f * aux) / 1856156927625.0f);//*///-ORIG

  aux = (xx = xx * x * x); //x^21//-ORIG 
  t = t + ((18888466084.0f * aux) / 194896477400625.0f);//*///-ORIG
...
//#define cot(x) (1.0 / tan(x)) //ORIG TAN
#define cot(x) (1.0 / taylor_tan(x))//taylor_TAN_t9
...
static void transit_surface() {
        ...
        if (radius_of_curvature != 0.0) {//if3
           if (object_distance == 0.0) {
              ...
              iang_sin = ray_height / radius_of_curvature;
               ...
               /*iang_sin = ((object_distance - radius_of_curvature) / 
                    radius_of_curvature) * sin(axis_slope_angle);//ORIG*/
                iang_sin = ((object_distance - radius_of_curvature) / radius_of_curvature) *
                    taylor_sin(axis_slope_angle);//*///taylor_SIN_t3         
           }
           iang = asin(iang_sin);
           ...
           //sagitta = sin((old_axis_slope_angle + iang) / 2.0);//ORIG
           sagitta = taylor_sin((old_axis_slope_angle + iang) / 2.0);//taylor_SIN_t4
           
           sagitta = 2.0 * radius_of_curvature*sagitta*sagitta;
           /*object_distance = ((radius_of_curvature * 
                               sin(old_axis_slope_angle + iang)) *
                               cot(axis_slope_angle)) + sagitta;//ORIG - SENO e TAN*/
            object_distance = ((radius_of_curvature * 
                               taylor_sin(old_axis_slope_angle + iang)) *
                               cot(axis_slope_angle)) + sagitta;//taylor - SENO_t5 e TAN_t9 */
           return;
           
        }//if3
          /*rang = -asin((from_index / to_index) *
                sin(axis_slope_angle));//ORIG*/
          rang = -asin((from_index / to_index) *
                taylor_sin(axis_slope_angle));//taylor_SIN_t6*/        
                
           
        /*object_distance = object_distance * ((to_index *
           cos(-rang)) / (from_index * cos(axis_slope_angle)));//ORIG*/
        /*object_distance = object_distance * ((to_index *
           taylor_cos(-rang)) / (from_index *
           cos(axis_slope_angle)));//taylor_COS_t1 */ 
         object_distance = object_distance * ((to_index *
           taylor_cos(-rang)) / (from_index *
           taylor_cos(axis_slope_angle)));//taylor_COS_t2 */ 
...
{//main
        ...
        //for (itercount = 0; itercount < niter; itercount++) {//for1
           ...
           /*aberr_osc = 1.0 - (od_sa[1][0] * od_sa[1][1]) /
              (sin(od_sa[0][1]) * od_sa[0][0]);//ORIG*/
           aberr_osc = 1.0 - (od_sa[1][0] * od_sa[1][1]) /
              (taylor_sin(od_sa[0][1]) * od_sa[0][0]);//taylor_SIN_t7 */ 
           //*********************************************************  
           aberr_lchrom = od_fline - od_cline;
           //*********************************************************
           //max_lspher = sin(od_sa[0][1]);//ORIG
           max_lspher = taylor_sin(od_sa[0][1]);//taylor_SIN_t8
           printf("%.3lf\n%.3lf\n",aberr_osc,max_lspher);//2 elementos
           //*********************************************************
